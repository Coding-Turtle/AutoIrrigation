...
def main():
    ...
    if checkConfig(configFields):
        ...
        #check if calibration fields are entered, if not enter calibration mode and get settings
        if (configFields["water"]) == "None" or (configFields["air"]) == "None":
            print("Calibration not filled, entering Calibration Mode")
            results = calibration.main()
            configFields["air"] = results[0]
            configFields["water"] = results[1]
            saveCheck = saveConfigUpdates(configFields)
            #check that config for calibration saved, if saved restart machine
            if saveCheck:
                print("Calibration added, Restarting")
                sleep(5)
                machine.reset()
            else:
                print("Calibration Save error, please disconnect device and Try again")
